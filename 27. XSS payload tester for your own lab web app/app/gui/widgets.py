"""Dark theme + reusable widgets for the XPT GUI.

Note: `add_text` and `make_tree` manage themselves with pack; always place
them inside a dedicated frame (never grid them directly).
"""
...
class Card(ttk.Frame):
    def __init__(self, master, title: str = "", **kw):
        ...
        if title:
            ttk.Label(self.inner, text=title, style="TLabel",
                      font=(FONT_FAMILY, 10, "bold"), foreground=ACCENT).pack(
                anchor="w", pady=(0, 6))
