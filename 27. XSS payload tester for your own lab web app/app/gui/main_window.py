"""XPT main window: controller, toolbar, tabs, event loop wiring.
...
from app.gui.tabs import (FindingsTab, ParametersTab, PayloadsTab,
                          ReportsTab, ScanTab, SettingsTab, TargetTab)
...
class XptApp:
    def __init__(self, root: tk.Tk):
        ...
        apply_theme(root)
        root.title(f"{APP_NAME} — XSS Payload Tester (lab web apps) v{__version__}")
        ...
        self._log(f"{APP_NAME} started. Lab targets only — confirm "
    ...
    def _build_toolbar(self) -> None:
        ...
        ttk.Label(bar, text="script " + APP_NAME, font=("Segoe UI", 15, "bold"),
                  foreground="#89b4fa").pack(side="left")
        ttk.Label(bar, text=" Browser-aware XSS testing for lab apps",
    ...
    def _build_tabs(self) -> None:
        ...
        self.notebook.add(self.payloads_tab, text="  3. Payloads  ")
        self.notebook.add(self.scan_tab, text="  4. Scan  ")
        self.notebook.add(self.findings_tab, text="  5. Findings  ")
        self.notebook.add(self.reports_tab, text="  6. Reports  ")
    ...
    def _load_last_config(self) -> None:
        t = {k: self.settings.get(k) for k in
             ("target_url", "method", "headers_text", "cookies_text",
              "body", "body_type")}
        self.target_tab.apply(t)
        p = {k: self.settings.get(k) for k in ("contexts", "custom_payloads")}
        self.payloads_tab.apply(p)
        s = {k: self.settings.get(k) for k in
             ("delay_ms", "timeout", "max_requests", "proxy", "verify_ssl")}
    ...
    def _gather_config(self) -> ScanConfig:
        ...
        cfg = ScanConfig(
            ...
            contexts=p["contexts"],
            custom_payloads=[l.strip() for l in p["custom_payloads"].splitlines()
                             if l.strip()],
            delay_ms=s["delay_ms"], timeout=s["timeout"],
            max_requests=s["max_requests"], verify_ssl=s["verify_ssl"],
            proxy=s["proxy"], authorized=s["authorized"],
        )
        params_raw = self.params_tab.get_all()
        if params_raw:
            cfg.parameters = [Parameter(
                location=p_["location"], name=p_["name"], value=p_["value"],
                enabled=p_["enabled"]) for p_ in params_raw]
        else:
    ...
    def _start_scan(self) -> None:
        ...
        if not cfg.authorized:
            messagebox.showwarning(APP_NAME,
                                   ...
                                   "This tool is for your own lab apps only.")
            return
    ...
    def _on_scan_finished(self) -> None:
        ...
        if self.current_scan_id is not None:
            self.db.finish_scan(self.current_scan_id, st.get("parameters", 0),
                                st.get("payloads", 0), st.get("requests", 0),
