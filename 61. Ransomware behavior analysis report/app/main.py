"""Malware Persistence Technique Cataloger - entry point."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog=APP["slug"], description=f"{APP['name']} v{APP['version']}")
    parser.add_argument("--analyze", metavar="PATH", help="analyse an artefact headlessly and exit")
    parser.add_argument("--demo", metavar="KIND", help="run a built-in demo scenario headlessly")
    parser.add_argument("--selftest", action="store_true", help="run every demo scenario and verify")
    parser.add_argument("--out", metavar="DIR", help="directory for exported reports")
    parser.add_argument("--formats", default="all", help="comma separated: html,pdf,json,csv,md,iocs.csv or 'all'")
...
def run_cli(args) -> int:
    ...
    formats = (["html", "pdf", "json", "csv", "md", "iocs.csv"]
    ...
    if args.selftest:
        ...
        if args.demo not in demo.DEMO_KINDS:
            print(f"Unknown demo '{args.demo}'. Available: {', '.join(demo.DEMO_KINDS)}", file=sys.stderr)
            return 2
