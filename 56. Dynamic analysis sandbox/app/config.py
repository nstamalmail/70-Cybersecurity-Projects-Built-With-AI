"""Dynamic Analysis Sandbox - application metadata and portable runtime paths.
...
APP: dict = {
    ...
    "subtitle": "Isolated VM orchestration \u00b7 behaviour telemetry \u00b7 timeline, process tree, network, IOCs",
    "version": "1.0.0",
    "vendor": "Malware Analysis Workbench",
    "report_title": "Dynamic Behaviour Report",
    "artifact_noun": "detonation",
    "accent": "#f5a524",
    "accent_dim": "#4a3416",
    "file_filters": [
        ("Behaviour logs / reports (*.json *.jsonl *.log)", "*.json *.jsonl *.log"),
        ("Executables (*.exe *.dll *.sys)", "*.exe *.dll *.sys"),
        ("All files (*)", "*"),
    ],
    "description": (
        "Orchestrates sandboxed detonation of suspicious samples in an isolated "
        "guest VM, normalises the behaviour telemetry (API calls, process tree, "
        "file system, registry, network) and produces a navigable behaviour report "
        "with extracted IOCs.  Runs against a real hypervisor when one is "
        "available and against an explicit simulation harness otherwise."
    ),
    "settings": {
        "analysis_timeout_s": 120,
        "vm_name": "win10-analysis",
        "snapshot": "clean-baseline",
        "hypervisor": "auto",          # auto | virtualbox | simulated
        "network_mode": "simulated",   # simulated | host-only | none
        "max_events": 500000,
