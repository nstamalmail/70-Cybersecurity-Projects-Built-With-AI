"""HTTP client + parameter discovery for SIDT.

Discovers injectable parameters from the URL query string, body, headers and
cookies; supports manual parameter JSON upload (architecture.md §3.1
ParameterInventoryView 'Manual parameter addition').
"""
...
USER_AGENT = "SIDT/1.0 (defensive SQLi scanner; authorized testing only)"
...
def parse_body_params(body: str, body_type: str) -> List[Tuple[str, str]]:
    """Extract key/value pairs from a request body (form or JSON)."""
...
def discover_parameters(cfg: ScanConfig) -> List[Parameter]:
    """Auto-discover injectable parameters from the configured request."""
    ...
    for k, v in cfg.headers.items():
        if k.lower() in ("referer", "user-agent", "x-forwarded-for", "x-forwarded-host"):
            params.append(Parameter("header", k, v))
    for k, v in cfg.cookies.items():
        params.append(Parameter("cookie", k, v))
    return params


def merge_manual_parameters(cfg: ScanConfig, doc: Any) -> List[Parameter]:
    """Merge manually uploaded parameter definitions into cfg.

    Accepts a list of {location,name,value,enabled} or {"parameters": [...]}.
    Returns the merged list (already applied to cfg.parameters).
    """
    items = doc.get("parameters", doc) if isinstance(doc, dict) else doc
    merged = {p.key(): p for p in cfg.parameters}
    if isinstance(items, list):
        for item in items:
            ...
            merged[p.key()] = p
    cfg.parameters = list(merged.values())
...
def build_request(cfg: ScanConfig, param: Parameter, value: str) -> Tuple[str, Dict, Dict, str]:
    ...
    if cfg.body_type == "json":
        ...
        pairs = [(k, v) for k, v in parse_body_params(cfg.body, cfg.body_type)]
        new_pairs = []
        replaced = False
        for k, v in pairs:
            if k == param.name and not replaced:
...
class HttpClient:
    """Rate-limited requests.Session wrapper."""
    ...
    def __init__(self, cfg: ScanConfig):
        ...
        self.delay_s = max(cfg.delay_ms, 0) / 1000.0
